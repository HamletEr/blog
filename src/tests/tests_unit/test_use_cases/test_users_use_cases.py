...
async def test_change_password(repo: AsyncMock, password_service: AsyncMock):
    user = make_user(hashed_password=OLD_HASHED_PASSWORD)
    ...
    result = await ChangePasswordUseCase(repo, user).execute(
        user_data, password_service
    )

    assert result == user
    assert result.hashed_password == NEW_HASHED_PASSWORD
    password_service.verify.assert_awaited_once_with(
        user_data.old_password, OLD_HASHED_PASSWORD
    )
    password_service.hash.assert_awaited_once_with(user_data.new_password)
    repo.update.assert_awaited_once_with(user)


async def test_change_password_raises_if_old_password_is_incorrect(
    repo: AsyncMock, password_service: AsyncMock
):
    user = make_user(hashed_password=OLD_HASHED_PASSWORD)
    user_data = ChangeUserPasswordCommand(
    ...
    with pytest.raises(IncorrectPassword):
        await ChangePasswordUseCase(repo, user).execute(user_data, password_service)

    password_service.hash.assert_not_awaited()
    repo.update.assert_not_awaited()


async def test_change_user_active_status(repo: AsyncMock):
    user = make_user(is_active=True)
    repo.get.return_value = user
    repo.update.side_effect = lambda updated_user: updated_user

    result = await ChangeUserActiveStatusUseCase(repo, user).execute(user.id)

    assert result == user
    assert result.is_active is False
    repo.get.assert_awaited_once_with(user_id=user.id)
    repo.update.assert_awaited_once_with(user)
